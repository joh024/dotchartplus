...
no matter what platform they work on.  This tool in not intended to
...
The [[Rnoweb]] function provides the user-callable interface to this
software.  It is written in \emph{closure} form to conceal the
collection of helper functions used to help carry out the
computations.  [[Rnoweb]] is the only user-visible component of this
software.
<<Rnoweb.R>>=
...
The [[Rnoweb]] function manages the entire \emph{weaving} and
...
in this vector, [[Rnoweb]] opens the file, reads its contents into
